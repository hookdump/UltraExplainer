---
description: Compare a plan/requirements against the codebase as a self-contained HTML audit
...
Read the plan and the relevant source. Build a point-by-point audit using `templates/swiss-comparison.html`: each requirement vs. what the code/plan actually does, with a status badge (match / partial / gap). Lead with the thesis (e.g. "N of M met — the gap is X"), KPI summary, a live-filterable table, and a collapsed detail for each gap. Anchor every "the plan does X" claim to the plan text or a `file:symbol`; mark inferences explicitly. Write the file, open it, summarize in one line.
